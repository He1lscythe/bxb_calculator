...
export const initFilterToggles = () => {
  ...
  document.getElementById('f-bunrui').innerHTML = renderFilterToggles('bunrui', BUNRUI_FILTER, {
    only: [1, 2, 3, 4, 5, 10, 12, 14, 15, 16, 19, 20],
  });
  document.getElementById('f-scope').innerHTML = renderFilterToggles('scope', SCOPE_SHORT);
  document.getElementById('f-condition').innerHTML = renderFilterToggles('condition', CONDITION, {
    skip: [0],
  });
};
...
export const applyFilters = () => {
  ...
  state.filteredBG = FilterCore.applyFilters(
    state.allBG,
    {
      q: q,
...
export const renderRowHd = (c) => {
  ...
  const checkCb = state.bgCheckEnabled
    ? '<input type="checkbox" class="bg-check-cb" data-id="' +
      c.id +
      '"' +
      (state.bgCheck.has(c.id) ? ' checked' : '') +
...
export const renderDetailBody = (c) => {
  ...
  return (
    ...
    '<img class="bg-icon" src="https://img.altema.jp/bxb/blade_graph/icon/' +
    c.id +
    '.jpg" onerror="this.style.display=\'none\'" alt="">' +
