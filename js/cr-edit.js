...
export const setCrystalHitStageScaling = (ei, idx, val) => {
  ...
  if (!e) return;
  const arr = (
    Array.isArray(e.hit_per_stage_scaling) ? e.hit_per_stage_scaling : [null, null, null]
  ).slice();
  while (arr.length < 3) arr.push(null);
  arr[idx] = _normalizeHitVal(val);
...
export const setCrystalStep = (kind, val) => {
  // kind: 'weight' | 'purity'
  const cr = state.editData;
  if (!cr) return;
...
export const setCrystalDelta = (ei, kind, val) => {
  // kind: 'weight' | 'purity' | 'lv'
  const e = state.editData?.effects?.[ei];
  if (!e) return;
  const deltaKey = kind + '_delta';
...
const _renderEffectCard = (e, i, total) => {
  ...
  const btogs = _CR_BUNRUI_ALL
    .map(function (b) {
      return (
        '<button class="btog' +
        (bunrui.includes(b) ? ' on' : '') +
        '" onclick="toggleCrystalBunrui(' +
        i +
        ',' +
        b +
        ')">' +
  ...
  const elemSel = renderEditSelect(
    { 0: '全属性', ...ELEMENT },
    e.element || 0,
    'setCrystalElement(' + i + ',this.value)',
  );
  const weapSel = renderEditSelect(
    { 0: '全武器種', ...WEAPON },
    e.weapon || 0,
    'setCrystalWeapon(' + i + ',this.value)',
  );
  const condSel = renderEditSelect(
    CONDITION,
    e.condition || 0,
    'state.editData.effects[' + i + '].condition=+this.value',
  );
  const ctSel = renderEditSelect(
    { 0: '×', 1: '+', 2: '+(終)', 3: '×(終)' },
    e.calc_type || 0,
    'state.editData.effects[' + i + '].calc_type=+this.value',
  );

  const bairituInitInput =
    '<input type="text" class="edit-num-sm" style="width:90px" value="' +
    (e.bairitu_init != null ? e.bairitu_init : '') +
    '" ' +
    'oninput="state.editData.effects[' +
    i +
    '].bairitu_init=parseBairituVal(this.value)">';
  const bairituInput =
    '<input id="edit-bairitu-' +
    i +
    '" type="text" class="edit-num-sm" style="width:90px" value="' +
    (e.bairitu != null ? e.bairitu : '') +
    '" ' +
    'oninput="setCrystalBairitu(' +
    i +
    ',this.value)">';

  const nameInput =
    e.scope === 5
      ? '<div class="field-label" style="margin-top:6px">キャラ名</div><input type="text" class="edit-input" style="width:100%" value="' +
        escHtml(e.name || '') +
        '" oninput="state.editData.effects[' +
        i +
        '].name=this.value">'
  ...
  const delBtn =
    total > 1
      ? '<button class="latent-rm-btn" onclick="removeCrystalEffect(' +
        i +
        ')" title="削除">×</button>'
      : '';
  ...
  if (bunrui.includes(7)) {
    ...
    const stageIn = function (idx) {
      ...
      return (
        ...
        '-' +
        idx +
        '" type="text" class="edit-num-sm" style="width:60px" value="' +
        v +
        '" oninput="setCrystalHitStage(' +
        i +
        ',' +
        idx +
        ',this.value)"></div>'
    ...
    const scaleIn = function (idx) {
      ...
      return (
        ...
        (idx + 1) +
        '撃+</div>' +
        '<input type="text" class="edit-num-sm" style="width:60px" value="' +
        v +
        '" oninput="setCrystalHitStageScaling(' +
        i +
        ',' +
        idx +
        ',this.value)"></div>'
    ...
    hitBlock =
      ...
      renderEditSelect(
        { 0: '0 減衰なし加算', 1: '1 ダメージ維持加算', 2: '2 乗算', 3: '3 設定値' },
        ht,
        'state.editData.effects[' + i + '].hit_type=+this.value',
      ) +
      '</div>' +
      stageIn(0) +
      stageIn(1) +
  ...
  const wDeltaInline =
    wStep > 0
      ? '<div><div class="field-label" title="0=0g時0倍, 1=不衰減, 留空=該維度不衰減; 支持小数/分数 (例 1/2)">重量 delta</div>' +
        '<input type="text" class="edit-num-sm" style="width:90px" value="' +
        (e.weight_delta != null ? e.weight_delta : '') +
        '"' +
        ' oninput="setCrystalDelta(' +
        i +
  ...
  const pDeltaInline =
    pStep > 0
      ? '<div><div class="field-label" title="0=0%時0倍, 1=不衰減, 留空=該維度不衰減; 支持小数/分数">純度 delta</div>' +
        '<input type="text" class="edit-num-sm" style="width:90px" value="' +
        (e.purity_delta != null ? e.purity_delta : '') +
        '"' +
        ' oninput="setCrystalDelta(' +
        i +
        ",'purity',this.value)\"></div>"
      : '';
  ...
  return (
    ...
    scopeSel +
    '</div>' +
    '<div><div class="field-label">属性</div>' +
    elemSel +
    '</div>' +
    '<div><div class="field-label">武器</div>' +
    weapSel +
    '</div>' +
...
const _renderStepSelect = (kind, current, opts) => {
  ...
  const items = opts
    ...
    .join('');
  return (
    '<select class="edit-select" onchange="setCrystalStep(\'' +
    kind +
    '\',this.value)">' +
    items +
    '</select>'
...
export const renderEditBody = (c) => {
  ...
  const effEdits = allEffects
    .map(function (e, i) {
      return _renderEffectCard(e, i, allEffects.length);
    })
    .join('');

  const roEffect = c.effect_text
    ? '<div class="field-row"><div class="field-key">効果</div><div class="field-val edit-ro">' +
      escHtml(c.effect_text) +
      '</div></div>'
    : '';
  const roTokushu = c['特殊条件']
    ? '<div class="field-row"><div class="field-key">特殊条件</div><div class="field-val edit-ro">' +
      escHtml(c['特殊条件']) +
      '</div></div>'
    : '';
  ...
  const lvBlock =
    ...
    '<input type="number" min="0" step="any" class="edit-num-sm" style="width:80px" value="' +
    (c.level_max != null ? c.level_max : '') +
    '"' +
    ' oninput="setCrystalLevelMax(this.value)"></div>';
  const wMinBlock =
    '<div><div class="field-label" title="0%重量を表す物理下限 (g)。空/0=下界 0g">weight_min (g)</div>' +
    '<input type="number" min="0" max="100" step="any" class="edit-num-sm" style="width:80px" value="' +
    (c.weight_min != null ? c.weight_min : '') +
    '"' +
    ' oninput="setCrystalWeightMin(this.value)"></div>';
  const wStepBlock =
    '<div><div class="field-label">weight_step (g)</div>' +
    _renderStepSelect('weight', c.weight_step, _WEIGHT_STEPS) +
    '</div>';
  const pStepBlock =
    '<div><div class="field-label">purity_step (%)</div>' +
    _renderStepSelect('purity', c.purity_step, _PURITY_STEPS) +
    '</div>';
  const topMeta =
  ...
  return (
    ...
    '</div>' +
    '<div class="edit-ro" style="font-size:13px;color:var(--text);padding:3px 0 8px">' +
    escHtml(c.name) +
    '</div>' +
    roEffect +
    roTokushu +
