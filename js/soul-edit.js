...
export const toggleElem = (path, id, btn) => {
  ...
  setPath(
    state.editData,
    path + '.element',
    arr.length === 0 ? null : arr.length === 1 ? arr[0] : arr,
  );
};
    ...
    .map((it) => {
      ...
      const effectBlocks = allEffects
        .map((e, ei) => {
          ...
          const selElems =
            e.element == null ? [] : Array.isArray(e.element) ? e.element : [e.element];
          const elemTogs = Object.entries(ELEMENT)
            .map(
              ([k, v]) =>
          ...
          const typeTogs = Object.entries(WEAPON)
            .map(
              ([k, v]) =>
          ...
          bunrui.includes(7)
            ? (() => {
                ...
                const hpss = Array.isArray(e.hit_per_stage_scaling)
                  ...
                  : [null, null, null];
                const stageIn = (i) =>
                  `<div><div class="field-label">${i + 1}撃</div><input type="text" class="edit-num-sm" style="width:42px" value="${hps[i] != null ? hps[i] : ''}" oninput="setHitStage('${ep}',${i},this.value)"></div>`;
                const scaleIn = (i) =>
                  `<div><div class="field-label">${i + 1}撃+</div><input type="text" class="edit-num-sm" style="width:42px" value="${hpss[i] != null ? hpss[i] : ''}" oninput="setHitStageScaling('${ep}',${i},this.value)"></div>`;
                return min`
