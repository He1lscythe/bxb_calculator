...
export const renderList = () => {
  ...
  list.innerHTML = state.filteredSouls
    .map(
      (s) => min`
...
export const navSoul = (dir) => {
  ...
  const newIdx =
    (((base + dir) % state.filteredSouls.length) + state.filteredSouls.length) %
    state.filteredSouls.length;
...
export const renderDetail = (s) => {
  ...
  return min`
    <div class="soul-header">
      ...
      ${
        (s.tags || []).length
          ? `<div class="soul-tags" style="display:flex;gap:4px;flex-wrap:wrap;margin-top:8px">
