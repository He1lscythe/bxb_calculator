...
export const renderList = () => {
  ...
  list.innerHTML = state.filteredChars
    .map((c) => {
      ...
      const dots = ['極弐', '改造', '通常']
        .map(
          (s) => `<div class="state-dot ${stateKeys.includes(s) ? 'has' : ''}" title="${s}"></div>`,
        )
        .join('');
...
export const navChara = (dir) => {
  ...
  const newIdx =
    (((base + dir) % state.filteredChars.length) + state.filteredChars.length) %
    state.filteredChars.length;
...
export const renderDetail = (c) => {
  ...
  const omoideBtn = hasOmoide(c)
    ? `<button class="btn-latent btn-omoide" onclick="openLatentModal(${c.id})">潜在開放</button>`
    : '';
  const masouBtn =
    state.masouByChara && (state.masouByChara[c.id] || []).length
      ? `<button class="btn-latent btn-masou" onclick="openMasouModal(${c.id})">魔装</button>`
      : '';
  const actionsHtml =
    omoideBtn || masouBtn ? `<div class="chara-actions">${masouBtn}${omoideBtn}</div>` : '';
...
export const renderStateContent = (sectionState, stateLabel, chara) => {
  ...
  if (allSkills.length) {
    const cards = allSkills
      .map(
        (s) =>
          '<div class="skill-card"><div class="skill-name-row"><span class="skill-name">' +
          escHtml(s.name) +
          '</span>' +
          renderRightTags(s) +
          '</div><div class="skill-effect-row"><span class="skill-effect">' +
          escHtml(s.effect_text) +
          '</span><span class="skill-bairitu">' +
          fmtBairitu(s) +
          '</span></div></div>',
      )
      .join('');
    parts.push(
      `<div class="section"><div class="section-title">スキル構成 (${allSkills.length})</div><div class="skills-list">${cards}</div></div>`,
    );
  }
  ...
  if (sectionState.basic_info) {
    parts.push(
      `<div class="section"><div class="section-title">基本情報</div>${renderBasicInfo(sectionState.basic_info)}</div>`,
    );
  }
    ...
    .map(([k, v]) => {
      ...
      if (k === 'Hit数' && Array.isArray(v)) {
        const phases = v
          .map(
            (n, i) =>
              `<span class="hit-phase">${n}</span>${i < v.length - 1 ? '<span class="hit-arrow">-</span>' : ''}`,
          )
          .join('');
