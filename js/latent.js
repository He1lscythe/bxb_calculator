...
export const _senzaiLabel = (iconId) => {
  ...
  const bVal =
    info.bairitu != null && info.bairitu !== 0
      ? ' (' + ctPfx(info.calc_type) + fmtNum(info.bairitu) + ')'
      : '';
...
export const getLinkedThresholds = (threshold, omoideRarity) => {
  ...
  let icon = [];
  for (let n = 0; n < 5; n++) {
    [18000, 21000, 27000].forEach(function (b) {
      s2h.push(b + 15000 * n);
    });
...
const _buildLCSOpts = (iconId, t, slotIdx, restricted) => {
  ...
  let keys = Object.keys(tbl).sort(function (a, b) {
  ...
  return keys
    .map(function (k) {
      ...
      const sel =
        iconId != null && parseInt(k) === iconId
          ? ' class="lcs-opt lcs-selected"'
          : ' class="lcs-opt"';
      return (
        '<div' +
        sel +
...
export const makeLCSHtml = (iconId, t, slotIdx, restricted) => {
  ...
  const searchHtml = restricted
    ? ''
    : '<input class="lcs-search" type="text" placeholder="絞込..." oninput="filterLCS(this)">';
  // data-* 属性で初回開閉時に _buildLCSOpts に必要な情報を保持。.lcs-list は初期空、
  // toggleLCS が初開時に populate。
  const iconAttr = iconId != null ? iconId : '';
  return (
    '<div class="latent-edit-chip">' +
    '<button class="lcs-trigger" data-icon="' +
    iconAttr +
    '" data-t="' +
    t +
    '" data-slot="' +
    ...
    '" data-rest="' +
    (restricted ? '1' : '0') +
    '" onclick="toggleLCS(event,this)">' +
    escHtml(triggerLabel) +
    '</button>' +
    '<div class="lcs-dropdown">' +
    searchHtml +
    '<div class="lcs-list"></div>' +
    '</div>' +
    '<button class="latent-rm-btn" onclick="removeLatentSlot(' +
    t +
    ',' +
    slotIdx +
    ')">×</button>' +
...
export const closeLCS = () => {
  if (state._lcsOpen) {
    state._lcsOpen.style.display = 'none';
    state._lcsOpen = null;
  }
};
...
export const filterLCS = (input) => {
  ...
  input
    .closest('.lcs-dropdown')
    .querySelectorAll('.lcs-opt')
...
export const selectLCS = (optEl, threshold, slotIdx) => {
  ...
  if (linked && linked.length > 1) {
    linked.forEach(function (t) {
      ...
      const row = (state.editData.omoide || []).find(function (r) {
...
export const fillOmoideSlots = (omoideArr, omoideRarity, k, star2High, star3High) => {
  const omap = {};
  (omoideArr || []).forEach((r) => {
    if (r.slots?.length) omap[r.threshold] = r.slots;
  });
  const star1 = omap[10] || [],
    star2 = omap[400] || [];
  ...
  const _s3icon = (x) =>
    x >= 54 && x <= 64 ? x + 4 : x >= 83 && x <= 87 ? x + 1 : x <= 82 ? x + 6 : x;
  ...
  else if (star2.length && !star3_raw.length) star3 = star2.map(_s3icon);
  else star3 = star3_raw;
  ...
  const rules = {};
  [2000, 3000, 5000, 7000, 9000, 13000].forEach((t) => {
    rules[t] = star1;
  });
  rules[4000] = star2;
  ...
  for (let n = 0; n < 5; n++) {
    ...
    rules[t30] =
      r === 5 ? [92] : r === 4 ? [91] : r === 3 ? [90] : r === 2 ? [89] : n === 0 ? s3h : [88];
  }
  ...
  return Object.keys(omap)
    .sort((a, b) => +a - +b)
    .map((t) => ({ threshold: +t, slots: omap[t] }))
    .filter((r) => r.slots.length);
};

export const renderOmoideTemplateBar = (c) => {
  const opts = state.omoideTemplates
    .map(
      (t) =>
...
export const saveOmoideTemplate = () => {
  ...
  if (!state.editData.omoide || !state.editData.omoide.length) {
    alert('潜在データがありません');
    return;
  }
  ...
  showPrompt('テンプレートを保存', existing?.name || 'テンプレート名', function (trimmed) {
    ...
    if (existing) {
      ...
      const newId = state.omoideTemplates.length
        ? Math.max(...state.omoideTemplates.map((t) => t.id)) + 1
        : 1;
    ...
    submitRevise({ omoide_templates: state.omoideTemplates })
      .then((j) => {
        ...
        const savedId = existing
          ? existing.id
          : state.omoideTemplates[state.omoideTemplates.length - 1].id;
        const bar = document.getElementById('omoide-tpl-sel');
        if (bar) {
          bar.innerHTML =
            '<option value="">テンプレートを選択...</option>' +
            state.omoideTemplates
              .map(
                (t) =>
...
const _syncTemplateSelect = () => {
  ...
  const same =
    tpl && JSON.stringify(state.editData.omoide || []) === JSON.stringify(tpl.omoide || []);
  if (same) return;
...
export const ensureOmoideRow = (threshold) => {
  ...
  let row = state.editData.omoide.find(function (r) {
    return r.threshold === threshold;
  });
  if (!row) {
    row = { threshold: threshold, slots: [] };
    state.editData.omoide.push(row);
    state.editData.omoide.sort(function (a, b) {
...
export const removeLatentSlot = (threshold, slotIdx) => {
  ...
  if (linked) {
    linked.forEach(function (t) {
      if (t === threshold) return;
      const r2 = (state.editData.omoide || []).find(function (r) {
        return r.threshold === t;
      });
...
export const openLatentModal = (charaId) => {
  const c = state.allChars.find(function (x) {
    return x.id === charaId;
  });
  ...
  function fmtBairituJP(info) {
    ...
    const sc =
      info.bairitu_scaling != null && info.bairitu_scaling !== 0
        ? fmtNum(info.bairitu_scaling)
        : '';
  ...
  const html = c.omoide
    .filter(function (row) {
      return row.slots && row.slots.length > 0;
    })
    .map(function (row) {
      const slotsHtml = row.slots
        .map(function (iconId) {
          ...
          const bDisplay = bStr || '—';
          return (
            '<div class="latent-slot">' +
            '<span class="latent-slot-koka">' +
            escHtml(info.koka || '?') +
            '</span>' +
            '<span class="latent-slot-syosai">' +
            escHtml(info.syosai || '') +
            '</span>' +
            '<span class="' +
            bClass +
        ...
        .join('');
      return (
        '<div class="omoide-row">' +
        '<div class="omoide-label">思い出 ' +
        row.threshold.toLocaleString('ja-JP') +
        '</div>' +
        '<div class="omoide-slots">' +
        slotsHtml +
        '</div>' +
        '</div>'
      );
