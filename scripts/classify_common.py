...
# 背水：残HPが少ない / 低い / 損傷率が高い / HPを消耗する / HPが減るほど のいずれもヒット。
# 「HPが減るほど」「残HPが減るほど」 variant を追加（id=44 等）。
_HAISUI   = re.compile(r'(?:残(?:り)?)?HP(?:が)?(?:少な|低)い(?:ほど|程)|HP残量が(?:少な|低)い(?:ほど|程)|損傷率が高い(?:ほど|程)|HPを?消耗する(?:ほど|程)|(?:残(?:り)?)?HP(?:が)?減る(?:ほど|程)')
...
# 「最終的に / 最終的な / 最後に」 → calc_type=2 (最終加算) / 3 (最終乗算)
# 「な」連体形は名詞を挟む可能性があるため 数字までの間に 0-8 文字の非数字非符号を許す
# 例：「最終的な攻撃力が2倍」「最終的な+50」「最終的に+30」「最後に1.5倍」
_V_FINAL_PLUS = re.compile(r'最(?:終的[にな]|後に)[^+＋\d]{0,8}?[+＋](\d+(?:\.\d+)?)')
_V_FINAL_BAI  = re.compile(r'最(?:終的[にな]|後に)[^\d]{0,8}?(\d+(?:\.\d+)?)倍')
...
def _extract_val_from_pos(text, pos, bunrui, force_plus_n=False):
    ...
    # 「最終的に / 最後に」 を最優先で候補に入れる。位置の早い候補が選ばれる仕組み上、
    # 「最終的に+N」「最終的にN倍」 が文中に現れれば普通の「+N」「N倍」 より優先される。
    m = _V_FINAL_PLUS.search(seg)
    if m: candidates.append((m.start(), float(m.group(1)),                              2))
    m = _V_FINAL_BAI.search(seg)
    if m: candidates.append((m.start(), float(m.group(1)),                              3))
...
def classify_skill_v2(skill, lookup_table, cat_to_bunrui):
    ...
    # cost_bunruis も union する：full text scan は condition trigger（残HP多いほど 等）で
    # bunrui=10 (HP) 検出が抑制される条件分岐があるため、cost-side 単独で検出した bunrui も
    # 必ず effects に積む。例：id=436 「最大HPを代償に、残HP多いほど攻撃力UP」では full-text の
    # 浑身 condition で HP 検出が走らないが、cost_text 「最大HPを」 単独 scan では bunrui=10 を拾う。
    for b in set(classify_effect(effect)['bunrui']) | cost_bunruis:
        if b in covered:
            continue
        effects.append(_make_entry(b))
        covered.add(b)
    ...
    # Veto: HP used as cost (消費) should not appear as bunrui=10 — these are
    # activation costs (HPを消費して攻撃する), not stat-affecting effects.
    # 但 cost-split (犠牲/代償) が既に fire していれば HP cost-side は合法な debuff
    # entry なので veto しない（cost text に明確に "HPを" 等が記述されている）。
    if _HP_COST.search(effect) and _cost_split is None:
    ...
    # Merge pass: 同じ (bairitu, calc_type, scope, condition, element, type) の entries は
    # bunrui[] を合体する（例「攻撃力とブレイク力とスピード77%UP」→ bunrui=[1,2,4] 単一 entry）。
    # crawl_chara.parse_bd_effects と同じ思想。
    # bunrui=[7] (hit) は hit_per_stage 等の追加 field が他 bunrui と整合しないので merge 除外。
    ...
    for e in effects:
        ...
        k = _merge_key(e)
        for m in merged:
            if 7 in (m.get('bunrui') or []) or 'name' in m: continue
            if _merge_key(m) == k:
...
_RARITY_MAX_JK = {1: 50, 2: 70, 3: 90, 4: 99}  # rarity → highest reachable jukudo

def classify_hit_fields(effect_text, ent, is_bd=False, rarity=4):
    ...
    rarity: chara rarity (1-4)。scaling 分数の分母は (rarity 別最高熟度 - 1)。
            未指定なら SS=4 と看做す（soul/crystal 等 rarity 不明な caller 用）。
    """
    ...
    if hit_type == 3:
        ...
        hit_per_stage = [n, n, n]
    elif (rng := re.search(r'(\d)[~〜～](\d)撃目[^+\-＋－0-9]{0,5}?([+\-＋－])(\d+)', text)):
        # 範囲表記「N〜M撃目+K」(NFKC 後 ～→~ なので ~ がメイン): [N..M] 段に ±K
        a, b, sign, num = rng.groups()
        v = int(num) * (-1 if sign in ('-', '－') else 1)
        lo, hi = int(a) - 1, int(b) - 1
        hit_per_stage = [v if lo <= i <= hi else 0 for i in range(3)]
    ...
    # hit_per_stage_scaling: extract from 【熟度a,b,c,...でさらに+N】 pattern.
    # 計算公式は scaling*(jukudo-1)。各 state の max_jukudo はマイルストーンリスト
    # 末尾の数値で自決定（SS 改造=99 / SS 通常=60 / A 改造=75 …）。
    # 保存値は分数字列 "{count*N}/{max-1}"。jk=max_jukudo で寄与は count*N に収束。
    sm = re.search(r'熟度([^】]*?)でさらに[+＋](\d+)', text)
    milestones = [int(x) for x in re.findall(r'\d+', sm.group(1))] if sm else []
    scaling_n  = int(sm.group(2)) if sm else 0
    if scaling_n > 0 and milestones:
        total_max  = len(milestones) * scaling_n
        # 分母は chara が（未来含めて）到達可能な最高熟度 - 1。
        # SS は改造で 99、A/B/C は極弐で 90/70/50。soul/crystal は SS 想定。
        denom      = _RARITY_MAX_JK.get(rarity, 99) - 1
        scaling_val = f'{total_max}/{denom}' if denom > 0 else 0
        hit_per_stage_scaling = [scaling_val if v != 0 else 0 for v in hit_per_stage]
        if all(s == 0 for s in hit_per_stage_scaling):
            hit_per_stage_scaling = [scaling_val, scaling_val, scaling_val]
    else:
