...
| `hit_per_stage` | bunrui=7时 | bunrui=7时 | — | bunrui=7时 | `int[3]` (type 0/1/3) / `float[3]` (type 2)，各段ヒット値 `[1撃目, 2撃目, 3撃目]` |
| `hit_per_stage_scaling` | bunrui=7时 | bunrui=7时 | — | bunrui=7时 | `int[3]`，每熟度 milestone 的増分（`[0,0,0]`=非 scaling）|
...
| 3 | 設定値 | `それぞれ1にする` / `1にする代わり` / `ヒット数を1` | 各段强制设为该 hit 数（int 绝对值） |
...
**bunrui=[7] 仅含 hit 时**：bairitu / bairitu_scaling 自动置 0（hit_per_stage 完全取代它们的语义）。混合 bunrui 如 `[7, 1]` 时 bairitu/bairitu_scaling 仍保留服务于其他 bunrui。

**hit_per_stage_scaling 提取规则**：
- 效果文匹配 `【熟度...でさらに+N】` 模式 → `hit_per_stage_scaling = [N, N, N]`（仅基础值非零的段）
- 游戏实际是连续公式 `value = base + (level-1)*(max-base)/98`，hit 数取整后视觉呈阶梯，wiki 用 milestone 表述。爬虫只抓表面 `+N`；复杂 milestone（如熟度 11,21,...90,99 累加 10 次）需手动 revise
- 非 scaling 的 hit skill 直接 `[0, 0, 0]`
...
| `1撃目2撃目のヒット数を代償に3撃目増加` | 0 | [0, 0, 0]（无法自动判断，需手动 revise） |
...
altema wiki 偶尔会**漏列**真实存在的技能或**多列**实际不存在的假技能。viewer 提供两套字段，用 revise 文件持久化用户的修正：

### `_deleted_skills`（墓碑）

按 skill 名字屏蔽 base 数据中的某条 skill。view 渲染时过滤；edit 模式仍可见（灰显），可点 ↻ 撤销。

| 数据源 | 位置 | 作用范围 |
|--------|------|---------|
| souls | `s._deleted_skills: string[]` | 整个魂 |
| characters | `c._deleted_skills: string[]` | 全 state（極弐/改造/通常 都过滤） |

### `_added_skills`（用户追加）

用户在 UI 里手动新增 wiki 漏列的真实 skill。爬虫不会触碰它，每次 crawl 应用 revise 时被恢复到合并结果中。

| 数据源 | 位置 | 作用范围 |
|--------|------|---------|
| souls | `s._added_skills: skill_obj[]` | 整个魂 |
| characters | `c._added_skills: { stateLabel: skill_obj[] }` | 仅指定 state |

字段结构与同源 base skill 一致（`name`/`effect`/`effects[]` 等）。

### 合并 / 渲染

| 阶段 | 行为 |
|------|------|
| crawl_*.py 的 `deep_update` | 把 revise 里 `_deleted_skills`/`_added_skills` 字段直接拷贝到 base 数据上（不做处理） |
| viewer 渲染 view | `skills.filter(name not in _deleted)` → concat `_added_skills` |
| viewer 渲染 edit | base + added 同时显示，base 被删的灰显（仍可 ↻ 撤销），added 标"追加"badge |

撤销/移除：
- 撤销 base 的删除 → `_deleted_skills` 数组移除该 name
- 移除一条 added skill → 从 `_added_skills` 数组/字典中 splice

`saveEdit` 在 diff 之前清理空数组/空字典，避免冗余字段进 revise。
...
| 文件 | 暴露 |
|------|------|
| `shared/filter-core.js` | `window.FilterCore = { applyFilters, toggleFilterValue, resetFilters }` |
| `shared/chara-spec.js` | `window.CHARA_SPEC` + `maxHit` / `maxBdhit` helpers |
| `shared/soul-spec.js` | `window.SOUL_SPEC` |
| `shared/crystal-spec.js` | `window.CRYSTAL_SPEC` + `_element` / `_weapon` / `_scope` 内部 helper |
| `shared/bg-spec.js` | `window.BG_SPEC` + `_element` / `_weapon` 内部 helper |

每个 viewer 通过 `<script src="shared/...">` 引入，HTML 顺序：`nav.js` → `filter-core.js` → 数据源 spec(s)。

### Spec 形态

```js
{
  searchFields: ['name', 'kana', ...],   // 搜索文本范围
  filters: {
    <key>: {
      // 三选一：
      extract: (item) => value,            // 默认 'eq' op：set.has(extract(item))
      op: 'any',                           // 'any' op：extract 返回数组，set ∩ array 非空
      match: (item, set) => boolean,       // 自定义谓词（覆盖 extract/op）
    },
    ...
  },
  sortFns: {
    <key>: (item) => number | null,        // 排序值（null 视为最小）
  },
}
```
...
{
  q: string,
  filters: { <key>: Set, ... },           // 每个 key 一个 Set，size=0 视为不启用该项
  sortKey: string,                        // 必须存在于 spec.sortFns 才生效
  sortDesc: boolean,                      // 默认 true
}
```
...
| 数据源 | filter keys |
|--------|-------------|
| chara  | `rarity`, `element`, `type`, `omoideRarity`, `state` (any), `bdSpecial` (any) |
| soul   | `rarity`, `element` (any), `type` (any) |
| crystal | `rarity`, `element`, `weapon`, `scope`, `bunrui` (match), `condition` (match) |
| bg     | `rarity`, `element`, `weapon`, `bunrui` (match), `scope` (match), `condition` (match) |

> 历史命名差异（已修正）：soul 的 `elem` 改为 `element`；crystal 的 `buki_type` 改为 `weapon`；hensei picker 的 soul `weapon` 改为 `type`。

### 调用方式

```js
filteredChars = FilterCore.applyFilters(allChars, {
  q: searchQuery,
  filters: filterSets,        // 现有 filterSets 对象直接传入
  sortKey: sortKey,
  sortDesc: sortDesc,
}, CHARA_SPEC);
```

各 viewer 自己负责 filter UI 的 DOM 渲染和默认排序（无 sortKey 时按 `sort_id` 或 `id` desc）。

### Hensei picker 的语义包装

hensei 的 picker 对 crystal `scope` 把 2/3/4/5 桶到一个「限定」组、对 bg/crystal 的 element/weapon 用 any-effect 匹配（不是 `_element`/`_weapon` 派生）。这两个差异通过 `HENSEI_BG_SPEC` / `HENSEI_CRYSTAL_SPEC` 在 `hensei.html` 内**包装**原 spec 实现：

```js
const HENSEI_CRYSTAL_SPEC = Object.assign({}, CRYSTAL_SPEC, {
  filters: Object.assign({}, CRYSTAL_SPEC.filters, {
    scope: { match: ... },
    element: { match: ... },
    weapon: { match: ... },
  }),
...
主 viewer 不受影响，picker 行为保持原样。

### 收益

- chara 的 `__hit_max` / `__bdhit_max` 排序自动可用于 hensei picker（已加入 dropdown）
- 加新 sort key：只改对应 `<spec>.js` 一处；通用 bug 修复：改 `filter-core.js` 一处
- 编辑流程（saveEdit / deepApply / 稀疏 revise）与 filter 系统**完全独立**，互不影响
