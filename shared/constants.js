...
const ELEMENT     = {0:'全属性',1:'火',2:'水',3:'風',4:'光',5:'闇',6:'無'};
const ELEM_COLOR  = {1:'#ff5533',2:'#3388ff',3:'#44cc66',4:'#ffcc00',5:'#aa55ff',6:'#888899'};
const ELEMS_ORDER = ['火','水','風','光','闇','無'];

const WEAPON        = {1:'長剣',2:'大剣',3:'太刀',4:'杖棒',5:'弓矢',6:'連弩',7:'戦斧',8:'騎槍',9:'投擲',10:'拳闘',11:'魔典',12:'大鎌'};
const WEAPONS_ORDER = ['長剣','大剣','太刀','杖棒','弓矢','連弩','戦斧','騎槍','投擲','拳闘','魔典','大鎌'];

const RARITY = {4:'SS',3:'S',2:'AA',1:'A'};
...
const BD_SPECIAL       = {1:'時止め',2:'麻痺',3:'強制ブレイク',5:'弱体解除',6:'高倍率バフ'};
const BD_SPECIAL_COLOR = {1:'#60c0ff',2:'#c060ff',3:'#ff9a40',5:'#50d070',6:'#ffc840'};

// 結晶 effects[].scope（详情/编辑下拉用长版，filter 按钮用短版）
const SCOPE       = {0:'自身/制限なし', 1:'セット全体', 2:'セット属性/武器限', 3:'自身属性/武器限', 5:'キャラ限定'};
const SCOPE_SHORT = {0:'自身',           1:'セット全',   2:'セット限',           3:'対象限',           5:'キャラ限'};

// effects[].condition：1=逆窮鼠(HP高) / 2=窮鼠(HP低) / 3=破損状態 / 4=敵ブレイク中
const CONDITION = {0:'なし', 1:'逆窮鼠', 2:'窮鼠', 3:'破損', 4:'敵ブレイク状態'};

// 元素 → CSS 颜色变量名（用于 filter 按钮 inline style.color）
const ELEM_CSS_VAR = {1:'--fire', 2:'--water', 3:'--wind', 4:'--light', 5:'--dark', 6:'--none'};

// 潜在開放（魔剣 omoide）熟度阈值，40 段
const OMOIDE_THRESHOLDS = [
...
function renderFilterToggles(field, map, opts) {
  opts = opts || {};
  return _pickKeys(map, opts).map(function(k){
    const attr = opts.attr ? opts.attr(+k) : '';
    return '<button class="ftog" data-val="' + k + '" onclick="toggleFilter(\'' + field + '\',' + k + ',this)"' + attr + '>' + map[k] + '</button>';
  }).join('');
}

// 元素 filter 专用：自动加 inline color 为对应元素 CSS 变量
function renderElementFilterToggles(field, opts) {
...
function renderEditSelect(map, currentVal, onchangeExpr, opts) {
  opts = opts || {};
...
function renderEditCheckboxes(map, selected, onchangeExpr, opts) {
  opts = opts || {};
