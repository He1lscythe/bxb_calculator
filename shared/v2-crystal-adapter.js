...
import {
  ...
  injectHitStages,
  MATH_TYPE_TO_CALC,
} from './v2-chara-adapter.js';
import { deepApply } from './v2-revise-core.js';
...
function _v2CrystalToWikiEffect(c) {
  ...
  const eff = {
    ...
    scope,
    condition,
    bairitu: c.max_value,                          // 最大值 (wiki_aux 给的)
    bairitu_init: c.initial_value,                 // 初期値 (master 直读)
    bairitu_scaling: 0,
    calc_type,
    weight_step: null,                             // master 无 (Phase 7 edit 手填)
    purity_step: null,
    _v2_parameter: c.parameter,                    // v2 原 parameter (cr-list renderEffLine 用)
  };
  if (c.element_id) {
...
export function v2CrystalToWiki(c) {
  ...
  return {
    _master: c,                            // 原 master entry (hensei stats-calc-v2 用)
    id: c.id,
    sort_id: c.id,
    name: c.name,
    rarity: c.rarity,
