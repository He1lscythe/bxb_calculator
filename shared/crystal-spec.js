...
const _crystalElement = (c) =>
  (c.effects || []).find((e) => (e.scope === 2 || e.scope === 3) && e.element != null)?.element ??
  0;

const _crystalWeapon = (c) =>
  (c.effects || []).find((e) => (e.scope === 2 || e.scope === 3) && e.weapon != null)?.weapon ?? 0;

const _crystalScope = (c) => ((c.effects || [])[0] || {}).scope || 0;
...
export const CRYSTAL_SPEC = {
  ...
  filters: {
    ...
    bunrui: {
      match: (c, set) =>
        [...set].some((v) => (c.effects || []).some((e) => (e.bunrui || []).indexOf(v) >= 0)),
    },
    condition: {
      match: (c, set) => [...set].some((v) => (c.effects || []).some((e) => e.condition === v)),
    },
  },
  sortFns: {
    rarity: (c) => c.rarity || 0,
