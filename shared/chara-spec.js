...
const _bestStateLabel = (c) => {
  if (!c?.states) return null;
  for (const s of ['極弐', '改造', '通常']) if (c.states[s]) return s;
  return null;
};
  ...
  for (const sk of state.skills || []) {
    for (const e of sk.effects || []) {
      ...
      for (let k = 0; k < N; k++) {
        ...
        if (ht === 3) {
          if (baseHit) stages[k] = baseHit;
        } else if (ht === 2) {
          if (baseHit) stages[k] = Math.floor(stages[k] * baseHit);
        } else {
          stages[k] += delta;
        }
      }
