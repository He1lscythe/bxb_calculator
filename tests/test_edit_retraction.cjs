...
eq(
  'tags 改了（无 prev） → 直接 emit 新 tags',
  buildDiffs(
    { id: 1, name: 'a', rarity: 4, tags: [1, 2] },
    { id: 1, name: 'a', rarity: 5, tags: [1, 2, 3] },
  ),
  { charDiff: { id: 1, name: 'a', rarity: 5, tags: [1, 2, 3] }, omoideDiff: null },
);

eq(
  '其他字段改 + tags 没动 + 无 prev → 不 emit tags（无 stale 可撤回）',
  buildDiffs(
    { id: 1, name: 'a', rarity: 4, tags: [1, 2] },
...
eq(
  '其他字段改 + tags 没动 + prev 有 tags=[1,2,3]（stale）→ emit tags=null 撤回',
...
eq(
  'base 无 tags + 修改其他字段 + 无 prev → 不写 tags（既无 base 又无 stale）',
...
eq(
  '只改 omoide_template（无 prev）→ omoide 由 template-override 注入 null；rarity 没动不注入',
  buildDiffs(
    { id: 1, name: 'a', omoide_rarity: 4, omoide: [{ threshold: 10, slots: [1] }] },
    {
      ...
      omoide_template: 5,
      omoide_rarity: 4,
      omoide: [{ threshold: 10, slots: [1] }],
    },
  ),
  { charDiff: null, omoideDiff: { id: 1, name: 'a', omoide_template: 5, omoide: null } },
);

eq(
  '改 omoide_rarity + template 没动 + 无 prev → 只 emit rarity',
  buildDiffs(
    { id: 1, name: 'a', omoide_rarity: 4, omoide: [{ threshold: 10, slots: [1] }] },
    { id: 1, name: 'a', omoide_rarity: 5, omoide: [{ threshold: 10, slots: [1] }] },
  ),
  { charDiff: null, omoideDiff: { id: 1, name: 'a', omoide_rarity: 5 } },
);

eq(
  '换 template（rarity 5→4 同 base）+ prev 有 rarity=5 stale → emit rarity=null 撤回',
  buildDiffs(
    { id: 1, name: 'a', omoide_rarity: 4, omoide: [{ threshold: 10, slots: [1] }] },
    {
      id: 1,
...
eq(
  'omoide 没改 + 只改 omoide_rarity + 无 prev → 只 emit rarity（不注入 omoide）',
  buildDiffs(
    { id: 1, name: 'a', omoide_rarity: 4, omoide: [{ threshold: 10, slots: [1] }] },
    { id: 1, name: 'a', omoide_rarity: 5, omoide: [{ threshold: 10, slots: [1] }] },
  ),
...
eq(
  'omoide 完全没改 → 整体不写 omoide_revise（无 prev tags 也不 emit）',
  buildDiffs(
...
eq(
  'apply template：omoide_template 正值 + omoide 数组 → omoide 强制 null',
  buildDiffs(
    { id: 1, name: 'a', omoide_rarity: 4, omoide: [{ threshold: 10, slots: [1] }] },
    {
      id: 1,
...
eq(
  '离开 template：omoide_template=null + omoide 数组改 + prev 有 template/rarity → emit 撤回',
  buildDiffs(
    { id: 1, name: 'a', omoide_rarity: 4, omoide: [{ threshold: 10, slots: [1] }] },
    {
      id: 1,
      name: 'a',
      omoide_template: null,
      omoide_rarity: 4,
...
eq(
  'rarity 改 + apply template（无 prev）→ charDiff 仅 rarity; omoideDiff 含 template/rarity',
  buildDiffs(
...
// 期望最终 revise 不残留 omoide_rarity=5
{
  ...
  eq('A1: 首次 apply → revise 有 template=5, rarity=5', revise, {
    id: 1,
    name: 'a',
    omoide_template: 5,
    omoide_rarity: 5,
  ...
  eq('A2: 切换后 revise 无 omoide_rarity 残留', revise, { id: 1, name: 'a', omoide_template: 2 });
}
...
// 场景 B：用户改 tags → 后撤回 tags 改其他字段，期望 tags 不残留
{
  ...
  revise = deepMerge(revise, d1);
  eq('B1: tags 改了 → revise 有 tags', revise, { id: 1, name: 'a', tags: [1, 2] });
  ...
  revise = deepMerge(revise, d2);
  eq('B2: tags 回 base + rarity 改 → revise.tags 被 pop，仅 rarity 保留', revise, {
    id: 1,
    name: 'a',
    rarity: 5,
  });
...
// 场景 C：apply template → 用户手改 slot 脱离 template
{
  ...
  const d1 = buildDiffs(base, ed1).omoideDiff;
  revise = deepMerge(revise, d1);
  eq('C1: apply template 3 → revise 有 template, rarity', revise, {
    id: 1,
    name: 'a',
    omoide_template: 3,
    omoide_rarity: 5,
  ...
  eq(
    'C2: 脱离 template → revise.omoide_template 被 pop, omoide 改动以 sparse-index 形式存',
    revise,
...
eq(
  'S1: base 无 tags → 添加 tags → revise 含 tags',
  soulSave({ id: 1, name: 'a' }, { id: 1, name: 'a', tags: [1, 2] }, undefined),
  { id: 1, name: 'a', tags: [1, 2] },
);

eq(
  'S2: revise.tags=[1,2] → 用户清空（editData.tags=[]） → normalize 后 diff vs prev 撤回，emit tags=null',
  soulSave(
    { id: 1, name: 'a' },
    { id: 1, name: 'a', tags: [] },
    { id: 1, name: 'a', tags: [1, 2] },
...
eq(
  'S3: base 无 tags + edit 也无 tags + 无 prev → 完全空，revise entry 整体不写入',
  soulSave({ id: 1, name: 'a' }, { id: 1, name: 'a' }, undefined),
  null,
);

eq(
  'S4: base 无 tags + editData.tags=[] + 无 prev → normalize 后无 tags，整 entry 不写入',
...
eq(
  'S5: revise.tags=[1,2] → edit 改为 [3] → diff emit 新值',
  soulSave(
    { id: 1, name: 'a' },
    { id: 1, name: 'a', tags: [3] },
    { id: 1, name: 'a', tags: [1, 2] },
...
eq(
  'S6: revise=undefined + edit 加 [1] + 再清空 [] → S1 后跟 S2 模式',
  soulSave({ id: 1, name: 'a' }, { id: 1, name: 'a', tags: [] }, { id: 1, name: 'a', tags: [1] }),
  { id: 1, name: 'a', tags: null },
);
...
eq(
  'B1: base 无 X → 加 X → revise 含 X',
  bgSave({ id: 1, name: 'a' }, { id: 1, name: 'a', element: 1 }, undefined),
  { id: 1, name: 'a', element: 1 },
);

eq(
  'B2: revise.element=1 → 用户改回 base（无 element）→ 撤回 emit element=null',
  bgSave({ id: 1, name: 'a' }, { id: 1, name: 'a' }, { id: 1, name: 'a', element: 1 }),
  { id: 1, name: 'a', element: null },
);

eq(
  'B3: 完全无差异 + 无 prev → null（整 entry 不写入）',
  bgSave({ id: 1, name: 'a' }, { id: 1, name: 'a' }, undefined),
  null,
);

eq(
  'B4: 多字段改 + partial 撤回 + 部分保留',
...
eq(
  'B5: effects 数组 sparse — 改 effect[1] 的 bairitu，effect[0] 不动',
  bgSave(
    {
      id: 1,
      name: 'a',
      effects: [
        { bunrui: [1], bairitu: 100 },
        { bunrui: [2], bairitu: 200 },
      ],
    },
    {
      id: 1,
...
eq(
  'B6: effects 数组 — 用户撤回 sparse 改动 → prev.effects[1].bairitu=999 → emit null 撤回',
  bgSave(
    {
      id: 1,
      name: 'a',
...
console.log('\n--- bladegraph 端到端：多次 save + deepMerge 落盘 ---');
{
  ...
  revise = deepMerge(revise, d1);
  eq('BG-E1: 加 element → revise 含 element', revise, { id: 1, name: 'a', element: 1 });

  // BG-E2: 撤回 element（用户保存「无变更」状态；prev 是 revise）
  const d2 = bgSave(base, { id: 1, name: 'a' }, revise);
  revise = deepMerge(revise, d2);
  eq('BG-E2: 撤回 → revise.element pop（剩 id/name 框架）', revise, { id: 1, name: 'a' });
}
...
eq(
  'C1: base 有 element=1 + scope=3 → 改 scope=0（UI 同步清 element=null）→ diff emit scope=0, element=null',
...
eq(
  'C2: revise 有 {scope:0, element:null} → 用户重新设 element=2 + scope 改回 base=3',
  crystalSave(
    { id: 1, name: 'a', effects: [{ scope: 3, element: 1, bairitu: 1 }] },
...
eq(
  'C3: 完全无差异 + 无 prev → null（不写 revise）',
  crystalSave(
...
eq(
  'C4: weight_step 撤回：base 无 weight_step → edit 设置 0.5 + delta=0.1 → emit 两者',
  crystalSave(
    { id: 1, name: 'a', effects: [{ bunrui: [1], bairitu: 1 }] },
    {
      ...
      name: 'a',
      weight_step: 0.5,
      effects: [{ bunrui: [1], bairitu: 1, weight_delta: 0.1 }],
    },
    undefined,
  ),
...
eq(
  'C5: revise 有 weight_step=0.5 → 用户清空（设 null）→ prev 撤回 emit null',
  crystalSave(
    { id: 1, name: 'a', effects: [{ bunrui: [1], bairitu: 1 }] },
    {
      id: 1,
      name: 'a',
      weight_step: null,
      effects: [{ bunrui: [1], bairitu: 1, weight_delta: null }],
    },
    { id: 1, name: 'a', weight_step: 0.5, effects: { 0: { weight_delta: 0.1 } } },
  ),
...
console.log('\n--- crystal 端到端：多次 save + deepMerge 落盘 ---');
{
  ...
  const d1 = crystalSave(
    base,
    { id: 1, name: 'a', effects: [{ scope: 0, element: null, bairitu: 1 }] },
    revise,
  );
  revise = deepMerge(revise, d1);
  eq('CR-E1: scope=0 + element=null → revise effects.0.scope=0（element 被 server pop）', revise, {
    id: 1,
    name: 'a',
    effects: { 0: { scope: 0 } },
  });
  ...
  revise = deepMerge(revise, d2);
  eq('CR-E2: 改回 base → revise.effects 全 pop（仅剩 id/name 框架）', revise, { id: 1, name: 'a' });
}
