...
let SENZAI_TABLE = {};
try {
  SENZAI_TABLE = load('senzai_table.json');
} catch (e) {}
...
function mkSlot(overrides) {
  return Object.assign(
    { chara: null, soul: null, bg: null, crystals: [], tr: mkTr() },
    overrides || {},
  );
}
...
const MOCK_A = {
  ...
  states: {
    通常: {
      ...
      skills: [
        { name: 'Self_攻+1000', effects: [{ bunrui: [1], scope: 0, calc_type: 1, bairitu: 1000 }] },
      ],
      basic_info: { 結晶スロット: 4 },
    },
    改造: {
  ...
  omoide: [
    { threshold: 100, slots: [9001] },
    { threshold: 5000, slots: [9002] },
  ],
};
...
const MOCK_B = {
  ...
  states: {
    通常: {
      ...
      skills: [
        { name: 'All_攻+500', effects: [{ bunrui: [1], scope: 1, calc_type: 1, bairitu: 500 }] },
      ],
    },
  },
};
  ...
  states: {
    通常: {
      ...
      skills: [
        {
          name: 'Fire_攻×1.20',
          effects: [{ bunrui: [1], scope: 2, element: 1, calc_type: 0, bairitu: 1.2 }],
        },
      ],
    },
  },
  ...
  states: {
    通常: {
      ...
      skills: [
        {
          name: 'Konshin_攻+2000',
          effects: [{ bunrui: [1], scope: 1, condition: 1, calc_type: 1, bairitu: 2000 }],
        },
      ],
    },
  },
  ...
  states: {
    通常: {
      ...
      skills: [
        {
          name: 'Hason_攻+3000',
          effects: [{ bunrui: [1], scope: 1, condition: 3, calc_type: 1, bairitu: 3000 }],
        },
      ],
    },
  },
  ...
  states: {
    通常: {
      ...
      skills: [
        {
          name: 'Konshin_攻×5',
          effects: [{ bunrui: [1], scope: 0, condition: 1, calc_type: 0, bairitu: 5 }],
        },
      ],
    },
  },
  ...
  states: {
    通常: {
      ...
      skills: [
        {
          name: 'Haisui_攻×7',
          effects: [{ bunrui: [1], scope: 0, condition: 2, calc_type: 0, bairitu: 7 }],
        },
      ],
    },
  },
...
const MOCK_SOUL_X = {
  ...
  rarity: 4,
  skills: [
    { name: 'Soul_攻+300', effects: [{ bunrui: [1], scope: 1, calc_type: 1, bairitu: 300 }] },
  ],
};
// mock bg
const MOCK_BG_X = {
  ...
  name: 'MOCK_BG',
  rarity: 4,
  effects: [{ bunrui: [1], scope: 1, calc_type: 0, bairitu: 1.1 }],
};
// mock crystal
const MOCK_CR_X = {
  id: 999301,
  name: 'MOCK_CR_FlatATK',
  rarity: 4,
  effects: [{ bunrui: [1], scope: 0, calc_type: 1, bairitu: 200 }],
};
const MOCK_CR_FIRE = {
  id: 999302,
  name: 'MOCK_CR_FireOnly',
  rarity: 4,
...
}
{
  ...
  t[0].tr = mkTr({
    ...
    level: 1,
    affinity: 5000,
    omoide_picks: { 100: 9001, 5000: 9002 },
  });
  const r = computeStats(MOCK_A, t[0].tr, t, 1);
...
}
{
  ...
  expectClose('C18 A 火属性受 C 加成', rA.stats['攻撃力'], 9900 * 1.2 + 1000 + 500);
  // B 不是火 → C 的乘不应用
...
}
{
  ...
  expectClose('C22 bg 全体乘', rA.stats['攻撃力'], (9900 + 1000) * 1.1);
}
...
// ----- 三魔剑 完整 -----
{
  ...
  const exp = ((9900 + 500) * 1.2 + 200 + 1000 + 500 + 300) * 1.1;
...
}
{
  ...
  const exp = ((9900 + 200 + 1000 + 500 + 500) * 1.2 + 300) * 1.1;
...
// R5: 全数据库扫描 — 每个 chara 默认 tr 不崩溃
{
  ...
  expect(
    `R5 全 ${realCharas.length} 魔剣 默认计算 数值正常`,
    statsOk,
    realCharas.filter((c) => c.states).length,
  );
}
...
{
  const ss = realCharas.find(
    (c) => c.rarity === 4 && c.states?.['通常']?.stats?.initial && c.states?.['通常']?.stats?.max,
  );
  if (ss) {
    const t = emptyTeam();
    t[0] = mkSlot({ chara: ss.id });
    ...
    for (const stKey of Object.keys(c.states)) {
      ...
      for (const sk of skills) {
        for (const e of sk.effects || []) {
          if ((e.bunrui || []).includes(17)) {
            found = { c, stKey, sk: sk.name };
            break;
          }
        }
        if (found) break;
...
// R11-R12: 找一个有 bairitu_scaling 的 chara skill, 验证 jk 增大时 buff 增大
{
  ...
  if (found) {
    ...
    expect(
      `R11 ${found.c.name} bairitu_scaling: jk大时 buff 更强`,
      rN.stats['攻撃力'] >= r1.stats['攻撃力'],
      true,
    );
  }
}
...
// R13: 找有 condition=2 (背水) 的 chara, hp=10 触发, hp=100 不触发
{
    ...
    for (const stKey of Object.keys(c.states)) {
      ...
      for (const sk of skills) {
        for (const e of sk.effects || []) {
          if (
            e.condition === 2 &&
            (e.bunrui || []).some((b) => _BUNRUI_TO_STAT[b]) &&
            (e.scope === 0 || e.scope === 1) &&
            (e.bairitu || 0) > 0
          ) {
            found = { c, stKey, ef: e };
  ...
  if (found) {
    ...
    expect(
      `R13 ${found.c.name} 背水 hp=10 ≥ hp=100 (背水触发)`,
      rLo.stats['攻撃力'] + 1e-6 >= rHi.stats['攻撃力'],
      true,
    );
  }
}
...
// R14: 全 souls 装备扫描 — 任一 chara 装备任一 soul 不崩溃
{
  ...
  if (baseCh) {
    ...
    for (const s of allSouls.filter((s) => s.id < 999000)) {
      ...
      try {
        computeStats(baseCh, t[0].tr, t, 1);
      } catch (e) {
        crashed++;
...
// R15: 全 crystals 装备扫描
{
  ...
  if (baseCh) {
    ...
    expect(
      `R15 全 ${allCrystals.filter((c) => c.id < 999000).length} 結晶 装备 不崩溃`,
      crashed,
      0,
    );
    if (failures.length) console.log('  样本失败:', failures);
...
// R16: 全 bg 装备扫描
{
  ...
  if (baseCh) {
    ...
    expect(
      `R16 全 ${allBGs.filter((b) => b.id < 999000).length} bladegraph 装备 不崩溃`,
      crashed,
      0,
    );
    if (failures.length) console.log('  样本失败:', failures);
  ...
  for (const c of realCharas) {
    ...
    for (const row of c.omoide) {
      for (const iconId of row.slots || []) {
        ...
        totalPicks++;
        try {
          computeStats(c, t[0].tr, t, 1);
        } catch (e) {
          crashed++;
          if (failures.length < 3)
            failures.push(`${c.id}/${c.name} thresh=${row.threshold} icon=${iconId}: ${e.message}`);
        }
      }
    }
  }
...
// R19: scope=2/3/4/5 实数据采样 — 跨所有 entity 类型找代表样本验证不崩溃
{
  ...
  const cr2 = allCrystals.find(
    (c) => c.id < 999000 && (c.effects || []).some((e) => e.scope === 2),
  );
  expect(
    `R19a scope=2 (crystal ${cr2?.name}) 不崩溃`,
    !!(
      cr2 &&
      (() => {
        try {
          tryWithCrystal(cr2.id);
          return true;
        } catch (e) {
          return false;
        }
  ...
  const so3 = allSouls.find(
    (s) =>
      s.id < 999000 && (s.skills || []).some((sk) => (sk.effects || []).some((e) => e.scope === 3)),
  );
  expect(
    `R19b scope=3 (soul ${so3?.name}) 不崩溃`,
    !!(
      so3 &&
      (() => {
        try {
  ...
  const cr3 = allCrystals.find(
    (c) => c.id < 999000 && (c.effects || []).some((e) => e.scope === 3),
  );
  expect(
    `R19c scope=3 (crystal ${cr3?.name}) 不崩溃`,
    !!(
      cr3 &&
      (() => {
        try {
          tryWithCrystal(cr3.id);
          return true;
        } catch (e) {
          return false;
        }
  ...
  const so4 = allSouls.find(
    (s) =>
      s.id < 999000 && (s.skills || []).some((sk) => (sk.effects || []).some((e) => e.scope === 4)),
  );
  expect(
    `R19d scope=4 (soul ${so4?.name}) 不崩溃`,
    !!(
      so4 &&
      (() => {
        try {
  ...
  const cr5 = allCrystals.find(
    (c) => c.id < 999000 && (c.effects || []).some((e) => e.scope === 5),
  );
  expect(
    `R19e scope=5 (crystal ${cr5?.name}) 不崩溃`,
    !!(
      cr5 &&
      (() => {
        try {
          tryWithCrystal(cr5.id);
          return true;
        } catch (e) {
          return false;
        }
...
results.forEach((r, i) => {
  ...
  mdRows.push(
    `| ${i + 1} | ${r.label} | ${fmt(r.expected)} | ${fmt(r.actual)} | ${r.ok ? '✓' : '✗'} |`,
  );
...
writeFileSync(
  ...
  '# 计算器测试结果\n\n' +
    ...
    '前 ' +
    results.findIndex((r) => r.label.startsWith('R')) +
    ' 项为受控 mock 验证（精确数值），其余为真实数据 sanity（含全数据库扫描和随机 fuzz）。\n\n' +
    '## 详细结果\n\n' +
    out +
    '\n' +
    coverageMd,
