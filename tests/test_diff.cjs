...
const charaBase  = {id:1647, name:'X', states:{改造:{skills:[{name:'真解放', effects:[{bairitu:3}]}]}}};
...
eq('bd_skill 跟 prev 等同 → 不进 diff（避免无脑刷）',
   ('bd_skill' in diff), false);
eq('states 撤回 → emit null',
   diff.states, {改造:{skills:{0:{effects:{0:{bairitu:null}}}}}});
...
eq('bd_skill 用户改动 → 应进 diff',
   diff2.bd_skill, {effects:{0:{bairitu:100}}});
...
eq('crystal: weight_min 跟 prev 同 → 不进 diff (_NOOP)',
   ('weight_min' in crDiff), false);
eq('crystal: effects[0].bairitu 撤回 → emit null',
   crDiff.effects, {0:{bairitu:null}});
...
eq('soul: element_affinity.火.atk_effect 跟 prev 同 → 不 emit',
   soulDiff2.element_affinity?.火?.atk_effect, undefined);
eq('soul: element_affinity.火.def_effect 撤回 → emit null',
   soulDiff2.element_affinity?.火?.def_effect, null);
...
eq('bg: effects[0].element 跟 prev 同 → 不 emit',
   bgDiff.effects?.[0]?.element, undefined);
eq('bg: effects[0].bairitu 撤回 → emit null',
   bgDiff.effects?.[0]?.bairitu, null);
