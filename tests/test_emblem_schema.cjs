...
const emblemLvMax = (rarity) => EMBLEM_RARITY_LV_MAX[+rarity || 1] || EMBLEM_RARITY_LV_MAX[1];
...
try {
  arr = JSON.parse(
    fs.readFileSync(path.join(__dirname, '..', 'data', 'guildemblems.json'), 'utf8'),
  );
} catch (e) {
...
for (const e of arr) {
  ...
  if (e.rarity != null && (e.rarity < 1 || e.rarity > 4))
    errs.push(`rarity=${e.rarity} 超出 [1,4]`);
...
truthy(
  `color 字段已普及（${Object.keys(colorCount).length} 种值）`,
