...
eq(
  'purity=0 → 1',
  effectiveBairitu({}, { weight: 100, purity: 0 }, { bairitu: 4, purity_delta: 0, calc_type: 0 }),
  1,
);
eq(
  'purity=10 → 1.3',
  effectiveBairitu({}, { weight: 100, purity: 10 }, { bairitu: 4, purity_delta: 0, calc_type: 0 }),
  1.3,
);
eq(
  'purity=100 → 4',
  effectiveBairitu({}, { weight: 100, purity: 100 }, { bairitu: 4, purity_delta: 0, calc_type: 0 }),
  4,
);
...
eq(
  ...
  effectiveBairitu(
    ...
    { bairitu: 5, weight_delta: 0.2, calc_type: 0 },
  ),
  5,
);
eq(
  'weight=0, delta=0.2 (mult) → 1.8',
  effectiveBairitu({}, { weight: 0, purity: 100 }, { bairitu: 5, weight_delta: 0.2, calc_type: 0 }),
  1.8,
);
eq(
  'weight=50, delta=0.2 (mult) → 3.4',
  effectiveBairitu(
    {},
    { weight: 50, purity: 100 },
    { bairitu: 5, weight_delta: 0.2, calc_type: 0 },
  ),
  3.4,
);
...
eq(
  ...
  effectiveBairitu(
    {},
    { weight: 100, purity: 100 },
    { bairitu: 5, weight_delta: 0.2, calc_type: 1 },
  ),
  5,
);
eq(
  'add weight=0, delta=0.2 → 1',
  effectiveBairitu({}, { weight: 0, purity: 100 }, { bairitu: 5, weight_delta: 0.2, calc_type: 1 }),
  1,
);
eq(
  'add weight=50, delta=0.2 → 3',
  effectiveBairitu(
    {},
    { weight: 50, purity: 100 },
    { bairitu: 5, weight_delta: 0.2, calc_type: 1 },
...
eq(
  'mult ct=0 delta=0 cfg=0 → 1 (neutral)',
  effectiveBairitu({}, { weight: 0, purity: 100 }, { bairitu: 3, weight_delta: 0, calc_type: 0 }),
  1,
);
eq(
  'add  ct=1 delta=0 cfg=0 → 0 (neutral)',
  effectiveBairitu({}, { weight: 0, purity: 100 }, { bairitu: 5, weight_delta: 0, calc_type: 1 }),
  0,
);
// mult delta=0 cfg=50 → factor=0.5 → eff = (3-1)·0.5+1 = 2
eq(
  'mult delta=0 cfg=50 → 2',
  effectiveBairitu({}, { weight: 50, purity: 100 }, { bairitu: 3, weight_delta: 0, calc_type: 0 }),
  2,
);
...
eq(
  'weight_delta 缺 → raw bairitu',
  effectiveBairitu({}, { weight: 0, purity: 100 }, { bairitu: 5, calc_type: 0 }),
  5,
);
eq(
  'weight_delta=null → raw bairitu',
  effectiveBairitu(
    {},
    { weight: 0, purity: 100 },
    { bairitu: 5, weight_delta: null, calc_type: 0 },
  ),
  5,
);
eq(
  'weight_delta="" → raw bairitu',
  effectiveBairitu({}, { weight: 0, purity: 100 }, { bairitu: 5, weight_delta: '', calc_type: 0 }),
  5,
);
...
eq(
  ...
  effectiveBairitu(
    { rarity: 4 },
    { weight: 100, purity: 100, lv: 120 },
    { bairitu: 2, lv_delta: 0.3, calc_type: 0 },
  ),
  2,
);
eq(
...
eq(
  'lv=60 (mult) → (2-1)·factor+1',
  effectiveBairitu(
    { rarity: 4 },
    { weight: 100, purity: 100, lv: 60 },
    { bairitu: 2, lv_delta: 0.3, calc_type: 0 },
  ),
  (2 - 1) * lvMid + 1,
);
...
eq(
  'lv=1, ct=1, lv_delta=0.5 → 5',
  effectiveBairitu(
    { rarity: 4 },
    { weight: 100, purity: 100, lv: 1 },
    { bairitu: 10, lv_delta: 0.5, calc_type: 1 },
  ),
  5,
);
...
eq(
  'max_lv=1 → factor_lv=1, raw bairitu',
  effectiveBairitu(
    { rarity: 99 },
    { weight: 100, purity: 100, lv: 1 },
    { bairitu: 3, lv_delta: 0, calc_type: 0 },
  ),
  3,
);
...
eq(
  ...
  effectiveBairitu(
    {},
    { weight: null, purity: 100 },
    { bairitu: 5, weight_delta: 0, calc_type: 0 },
  ),
  5,
);
eq(
  'cfg.lv==null + 满 max_lv → 不衰减 (lvVal=max_lv)',
  effectiveBairitu(
    { rarity: 4 },
    { weight: 100, purity: 100 },
    { bairitu: 3, lv_delta: 0.5, calc_type: 0 },
  ),
  3,
);
...
eq(
  'mult debuff cfg=0 → 1',
  effectiveBairitu({}, { weight: 0, purity: 100 }, { bairitu: 0.5, weight_delta: 0, calc_type: 0 }),
  1,
);
eq(
  'mult debuff cfg=50 → 0.75',
  effectiveBairitu(
    {},
    { weight: 50, purity: 100 },
    { bairitu: 0.5, weight_delta: 0, calc_type: 0 },
  ),
  0.75,
);
eq(
  'mult debuff cfg=100 → 0.5',
  effectiveBairitu(
    {},
    { weight: 100, purity: 100 },
    { bairitu: 0.5, weight_delta: 0, calc_type: 0 },
...
eq(
  'w_delta="1/2" (mult) cfg=50 → 4',
  effectiveBairitu(
    {},
    { weight: 50, purity: 100 },
    { bairitu: 5, weight_delta: '1/2', calc_type: 0 },
  ),
  4,
);
// purity_delta="1/4" = 0.25, ct=1, purity=60 → factor=0.25+0.75·0.6=0.7, eff=3·0.7=2.1
eq(
  'p_delta="1/4" (add) cfg=60 → 2.1',
  effectiveBairitu(
    {},
    { weight: 100, purity: 60 },
    { bairitu: 3, purity_delta: '1/4', calc_type: 1 },
  ),
  2.1,
);
// lv_delta="2/5" = 0.4, ct=0, rarity=4, lv=1 → factor=0.4, eff=(3-1)·0.4+1=1.8
eq(
  'lv_delta="2/5" (mult) lv=1 → 1.8',
  effectiveBairitu(
    { rarity: 4 },
    { weight: 100, purity: 100, lv: 1 },
    { bairitu: 3, lv_delta: '2/5', calc_type: 0 },
  ),
  1.8,
);
...
const r1 = effectiveBairitu(
  { weight_step: 10 },
  { weight: 50, purity: 100 },
  { bairitu: 5, weight_delta: 0.5, calc_type: 0 },
);
const r2 = effectiveBairitu(
  { weight_step: 100 },
  { weight: 50, purity: 100 },
  { bairitu: 5, weight_delta: 0.5, calc_type: 0 },
);
const r3 = effectiveBairitu(
  {},
  { weight: 50, purity: 100 },
  { bairitu: 5, weight_delta: 0.5, calc_type: 0 },
);
...
eq(
  '无 delta 字段 → raw 分式 "3/2" 保持',
  effectiveBairitu({}, { weight: 100, purity: 100 }, { bairitu: '3/2', calc_type: 0 }),
  '3/2',
);
eq(
  'cfg 满 + delta=0 → raw bairitu',
  effectiveBairitu({}, { weight: 100, purity: 100 }, { bairitu: 5, weight_delta: 0, calc_type: 0 }),
  5,
);
...
eq(
  'wMin=10, w=10 → 端点 factor=delta',
  effectiveBairitu(
    { weight_min: 10 },
    { weight: 10, purity: 100 },
    { bairitu: 5, weight_delta: 0.2, calc_type: 0 },
  ),
  1.8,
);
eq(
  'wMin=10, w=100 → factor=1 (raw)',
  effectiveBairitu(
    { weight_min: 10 },
    { weight: 100, purity: 100 },
    { bairitu: 5, weight_delta: 0.2, calc_type: 0 },
...
eq(
  'wMin=10, w=55 (中点) → factor=0.6',
  effectiveBairitu(
    { weight_min: 10 },
    { weight: 55, purity: 100 },
    { bairitu: 5, weight_delta: 0.2, calc_type: 0 },
  ),
  3.4,
);
...
// 三维叠加 + weight_min：wMin=20, w=60 (wPos=0.5)
{
  ...
  eq(
    '三维叠加 + wMin=20',
    effectiveBairitu(
      { weight_min: 20, rarity: 4 },
      { weight: 60, purity: 50, lv: 60 },
      { bairitu: 4, weight_delta: 0.5, purity_delta: 0.5, lv_delta: 0.5, calc_type: 0 },
    ),
    expected,
  );
...
eq(
  'wMin=100 退化 → factor_w=1, raw bairitu',
  effectiveBairitu(
    { weight_min: 100 },
    { weight: 50, purity: 100 },
    { bairitu: 5, weight_delta: 0.5, calc_type: 0 },
