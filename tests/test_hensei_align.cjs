...
const fmtHitStages = (e) => {
  ...
  const parts = hps
    .map((v, i) => (v == null || v === 0 ? null : i + 1 + '撃' + stageStr(v, sc[i] || 0)))
    .filter(Boolean);
...
const fmtScopeTag = (ef) => {
  ...
  if (ef.scope === 3)
    return '<span class="scope-tag scope-equip-s">' + (lim || '装') + '·自</span>';
  if (ef.scope === 4)
    return '<span class="scope-tag scope-equip-a">' + (lim || '装') + '·全</span>';
  return '';
};
...
eq(
  'scope=2 element=5 → 闇',
  fmtScopeTag({ scope: 2, element: 5 }),
  '<span class="scope-tag scope-lim">闇</span>',
);
eq(
  'scope=2 element=5 weapon=1 → 闇·剣',
  fmtScopeTag({ scope: 2, element: 5, weapon: 1 }),
  '<span class="scope-tag scope-lim">闇·剣</span>',
);
eq(
  'scope=3 element=5 → 闇·自',
  fmtScopeTag({ scope: 3, element: 5 }),
  '<span class="scope-tag scope-equip-s">闇·自</span>',
);
eq(
  'scope=4 weapon=1 → 剣·全',
  fmtScopeTag({ scope: 4, weapon: 1 }),
  '<span class="scope-tag scope-equip-a">剣·全</span>',
);
eq(
  'scope=2 (无属性) → 限',
  fmtScopeTag({ scope: 2 }),
  '<span class="scope-tag scope-lim">限</span>',
);
